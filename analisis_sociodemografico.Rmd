...
### <b style="font-size: 22px;">Comparación del uso diario de redes sociales.</b>

Al realizar este tipo de comparaciones no se observan diferencias resaltables entre estas cohortes comparadas. Estas cohortes fueron definidas a partir de las distribuciones observadas en la piramide poblacional, en donde una gran parte de los encuestados se encuentra en un primer intervalo de 18-20 años y otra en un segundo intervalo de 21-24 años. 

<div style="position: relative;">

```{r box-plots-demograficos, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
# Traducir género
datos_estudiantes$Genero <- ifelse(datos_estudiantes$Gender == "Female", "Femenino", "Masculino")

# Crear rangos de edad
datos_estudiantes$Rango_Edad <- cut(datos_estudiantes$Age, 
                                     breaks = c(16, 20, 24), 
                                     labels = c("17-20", "21-24"),
                                     include.lowest = TRUE)

# Gráfico A: Por género
plot_A <- ggplot(datos_estudiantes, aes(x = Genero, y = Avg_Daily_Usage_Hours, fill = Genero)) +
  geom_boxplot(width = 0.5, outlier.shape = 16, outlier.size = 2) +
  scale_fill_manual(values = c("Femenino" = "#F8A5A5", "Masculino" = "#A5C4E8")) +
  labs(x = "Género", y = "Horas promedio de uso diario") +
  theme_minimal() +
  theme(
...
# Gráfico B: Por rango de edad
plot_B <- ggplot(datos_estudiantes, aes(x = Rango_Edad, y = Avg_Daily_Usage_Hours, fill = Rango_Edad)) +
  geom_boxplot(width = 0.5, outlier.shape = 16, outlier.size = 2) +
  scale_fill_manual(values = c("17-20" = "#F8A5A5", "21-24" = "#A5C4E8")) +
  labs(x = "Rango de edad", y = "Horas promedio de uso diario") +
  theme_minimal() +
  theme(
    legend.position = "none",
    panel.grid.major.x = element_blank(),
...
```{r piramide-poblacional, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
